...
{\normalsize Meruva Kodanda Suraj, Mamta Nag, and Mallikharjuna Rao K.}\\[0.25\baselineskip]
...
Before turning to the individual points, we summarize the five
substantive changes that the revision makes. The last of these
corrects a defect neither report named explicitly; it is set out in
full in the section ``Additional Corrections Not Raised in the
Reviews.''
...
\item \textbf{Seven citations that resolved to the wrong references have
been corrected.} In the submitted version, most of the citations in the
Related Works section named works that were not the entries cited.
Section~II has been rewritten, every citation has been verified against
its entry, two entries have been replaced, and the bibliography has been
reformatted into the numbered IEEE style.
...
%==============================================================
\section*{Additional Corrections Not Raised in the Reviews}
\addcontentsline{toc}{section}{Additional Corrections Not Raised in the Reviews}

In auditing the manuscript against both reports we found a defect that
neither reviewer named explicitly, but which we believe contributed to
the rating both gave to the reference list. We report and correct it here
rather than leave it to be discovered later.

%--------------------------------------------------------------
\point{Additional Correction A.1. Citations in Section II did not resolve to the works discussed.}

\begin{reviewercomment}
5. The references are sufficient and up-to-date

Moderate \quad (Reviewer 3 and Reviewer 5)
\end{reviewercomment}

\response On re-checking every citation in the submitted version against
its bibliography entry, we found that seven of the eleven citations in
the Related Works section pointed to entries that had nothing to do with
the work being described. The text discussed studies by Nijhawan et al.,
Rastogi et al., Bobade et al., Zhu et al., Moosavi et al., Saylam et al.,
and Sharma et al.\ on social-media and wearable stress detection, but the
corresponding bibliography positions held, respectively, Livingstone and
Russo (RAVDESS), Tan and Le (EfficientNet), Gal and Ghahramani (Monte
Carlo Dropout), Guo et al.\ (calibration), Lakshminarayanan et al.\ (deep
ensembles), Geifman and El-Yaniv (selective classification), and Kendall
and Gal (uncertainty types). Only the first four citations in the section
resolved correctly.

The effect was that the submitted manuscript discussed seven papers that
did not appear in its bibliography at all, while its bibliography listed
seven papers the text never discussed. This is a serious error. It
misattributes published work, and any reviewer who checked one of those
citations would reasonably have concluded that the related-work
discussion was unreliable. We apologize for it. The cause was a
wholesale replacement of the Related Works narrative late in preparation
without a corresponding update to the bibliography, and we did not
re-verify the citations afterwards.

We have since checked all fifteen citations in the revised manuscript
individually against their entries, and every one resolves to the work
named in the text.

\begin{changesblock}
Section~II has been rewritten so that each cited work is the
work actually discussed, and all fifteen citations have been verified
individually against the bibliography. Two entries that the revised text
no longer discusses have been removed: Awada et al.\ (\emph{IEEE
Transactions on Affective Computing}, 2023) and Hochreiter and
Schmidhuber (\emph{Neural Computation}, 1997). Two entries have been
added to support the revised discussion: Mentis, Lee, and Roussos
(\emph{Molecular Psychiatry}, 2024) on deep learning for mental health
monitoring, and Nag, Vijaya, and Rao (AICCONF, 2025) on optimization for
occupational stress prediction. The bibliography remains at fifteen
entries. Separately, all entries have been reformatted from the
author-prominent style used in the submission into the numbered IEEE
style required by the proceedings, with initials preceding surnames and
volume, number, page, and year fields given explicitly.
\end{changesblock}
...
reported in Table~I. We have additionally corrected, and disclosed
above, a citation defect that neither report named: seven references in
Section~II of the submitted version did not resolve to the works the
text discussed.
